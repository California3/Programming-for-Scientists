...
def neural_network(x1, x2, x3, w1, w2, w3, w4, w5, w6, b1, b2, b3):
    # Sum of inputs to each neuron
    NN1_input = x1*w1 + x2*w2 + b1
    NN2_input = x2*w3 + x3*w4 + b2

    # ReLU activation function
    NN1_output = max(0, NN1_input)
    NN2_output = max(0, NN2_input)

    # Sum of hidden layer outputs
    NN3_input = NN1_output*w5 + NN2_output*w6 + b3

    # Sigmoid activation function
    NN3_output = 1/(1 + math.exp(-NN3_input))
