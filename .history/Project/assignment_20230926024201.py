...
def orbit_time(path_to_file, object_name):
    ...
    last_not_over = 0.0
    while True:
        position_forward = forward_simulation(path_to_file, T_time, int(T_time / T_step))
        distance = (position_forward[-1, 2*object_index] - object_position[0])**2 + (position_forward[-1, 2*object_index+1] - object_position[1])**2
        allow_distance = (position_forward[0, 2*object_index] - object_position[0])**2 + (position_forward[0, 2*object_index+1] - object_position[1])**2
        if distance <= epsilon:
            print(T_time, object_name, "small")
            last_not_over = T_time
            if last_over is None:
                T_time = T_time * 2
            elif last_over - last_not_over <= 1:
                return last_not_over
            else:
                T_time = ((T_time + last_over) / 2)
        else:
            last_over = T_time
            T_time = ((T_time + last_not_over) / 2)
            print(T_time, object_name, "large")
    return T_time
