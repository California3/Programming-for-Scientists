...
def forward_simulation(path_to_file, total_time, num_steps):
    ...
    position_forward[:,0:2] = compute_position(path_to_file, t_split)
    acceleration_init = compute_acceleration(path_to_file, t_split)
    velocity_current = np.zeros((data.shape[0], 2))
    
    velocity_current[:,0] = data[:, 3] + t_split * acceleration_init[:,0]
    velocity_current[:,1] = data[:, 4] + t_split * acceleration_init[:,1]
