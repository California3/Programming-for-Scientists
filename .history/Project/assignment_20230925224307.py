...
def forward_simulation(path_to_file, total_time, num_steps):
    ...
    data = np.loadtxt(path_to_file, skiprows=1, usecols=[_ for _ in range(1,5+1)])
    position_forward = np.zeros((data.shape[0], 2 * num_steps))

    velocity_current = np.zeros((data.shape[0], 2))
    position_current = compute_position(path_to_file, t_split)
    acceleration_current = compute_acceleration(path_to_file, t_split)
    
    velocity_current[:,0] = data[:, 3] + t_split * acceleration_current[:,0]
    velocity_current[:,1] = data[:, 4] + t_split * acceleration_current[:,1]
    position_forward[:,0:2] = position_current
    
    for step in range(1, num_steps):
        position_next = np.zeros((data.shape[0], 2))
        position_next = position_current + t_split * velocity_current

        acceleration_next = np.zeros((data.shape[0], 2))
        for planet_current in range(data.shape[0]):
            for planet_other in range(data.shape[0]):
                if planet_other == planet_current:
                    continue
        ...
        velocity_next = velocity_current + t_split * acceleration_next
        position_forward[:,2*step:2*step+2] = position_next
