...
def max_accurate_time(path_to_file, epsilon):
    current_i = 1
    while(True):
        position_forward_T = forward_simulation(path_to_file, current_i ** 2, current_i)
        position_forward_1 = forward_simulation(path_to_file, current_i ** 2, 1)
        error = np.linalg.norm(position_forward_T[-1] - position_forward_1[-1])
        print(error, current_i)
        if error > epsilon:
            return current_i - 1
        current_i += 1
