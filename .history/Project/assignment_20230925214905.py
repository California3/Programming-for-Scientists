    ...
    for planet_current in range(data.shape[0]):
        for planet_other in range(data.shape[0]):
            if planet_other == planet_current:
                continue
            pji_x = data[planet_other, 1] - data[planet_current, 1] # xj - xi
            pji_y = data[planet_other, 2] - data[planet_current, 2] # yj - yi
            distance = np.sqrt(pji_x**2 + pji_y**2) # sqrt[(xi - xj)**2 + (yi - yj)**2]

            force_ij = (G_const * data[planet_current, 0] * data[planet_other, 0] / distance**2)
            acceleration[planet_current, 0] += force_ij * pji_x / distance
