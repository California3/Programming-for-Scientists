...
def forward_simulation(path_to_file, total_time, num_steps):
    position_forward, position_current, velocity_current, acceleration_current = forward_simulation_speedup(path_to_file, total_time, num_steps)
    
    return position_forward

def forward_simulation_speedup(path_to_file, total_time, num_steps):
    G_const = 6.67e-11
    data = get_data(path_to_file)
    if num_steps == 0:
        t_split = 0
