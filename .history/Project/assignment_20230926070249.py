...
def orbit_time(path_to_file, object_name):
    ...
    last_distance
    while(True):
        if searching_step <= step_max:
            object_position_after = position_forward_T_cal[:searching_step, object_index*2:object_index*2+2][-1]
        else:
            position_forward_T_x2, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, searching_step - step_max, T_step, data)
            position_forward_T_cal = np.vstack((position_forward_T_cal, position_forward_T_x2))
            object_position_after = position_forward_T_cal[:step, object_index*2:object_index*2+2][-1]
            step_max = searching_step
