...
def max_accurate_time(path_to_file, epsilon):
    T = 1
    while(True):
        position_forward_T = forward_simulation(path_to_file, T, T)
        position_forward_1 = forward_simulation(path_to_file, T, 1)
        error = 0
        for i in range(1, position_forward_T.shape[1], 2):
            error += np.linalg.norm(position_forward_T[:,i-1:i+1] - position_forward_1[:,i-1:i+1])
        print(error, T)
        if error > epsilon:
            return T - 1
        T += 1
