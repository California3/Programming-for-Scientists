...
def max_accurate_time(path_to_file, epsilon):
    ...
    while(True):
        if T <= T_cal_max:
            ...
            position_forward_T_cal = np.vstack((position_forward_T_1, position_forward_T_x2))
            position_forward_T = position_forward_T_cal[:T, :][-1]
            T_cal_max = T
...
def orbit_time(path_to_file, object_name):
    ...
    step_max = 400000
    position_forward_full = forward_simulation(path_to_file, T_step * step_max, step_max)[:, object_index*2:object_index*2+2]
    while(True):
        iter_cnt += 1
        restart = False
        if step_by_step < 500:
        ...
        if step >= step_max:
            position_forward = forward_simulation(path_to_file, T, step)
            object_position_after = position_forward[-1, object_index*2:object_index*2+2]
        else:
            position_forward = position_forward_full[:step, :]
