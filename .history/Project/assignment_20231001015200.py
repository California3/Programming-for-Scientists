    ...
    while(True):
        ...
        if error <= epsilon:
            ...
            if last_over is None:
                ...
                # print("\n \033[42m","Boundary is found at T: ", last_not_over, "Max iteration: ", iter_cnt, "\033[0m \n")
                return last_not_over
            else:
                T = int((T + last_over) / 2)
            # print reach high boundary at T
            # print("Iter:", iter_cnt,"Current T:", T, "Upgrade low boundary at T.")
        else:
            # print reach high boundary at T
            # print("Iter:", iter_cnt,"Current T:", T, "Downgrade high boundary at T.")
            last_over = T
            T = int((T + last_not_over) / 2)
        iter_cnt += 1

def orbit_time_epsilon(path_to_file, object_position, object_name, object_index, epsilon):
    """
    Calculates the time it takes for an object to orbit around a celestial body with a given epsilon (accuracy).

    Args:
        path_to_file (str): The path to the file containing the data for the simulation.
        object_position (Tuple[float, float]): The initial position of the object.
        object_name (str): The name of the object being orbited.
        object_index (int): The index of the object being orbited in the simulation data.
        epsilon (float): The desired accuracy of the orbit time calculation.
    Returns:
        Tuple[bool, Optional[float]]: A tuple containing a boolean indicating whether the calculation was successful and the calculated orbit time (if successful).
    """
    ...
    position_forward_T_1, position_current, velocity_current, acceleration_current, data = forward_simulation_speedup(path_to_file, 1, T_step)
    step_max = 10
...
def orbit_time(path_to_file, object_name):
    ...
    Args:
    - path_to_file (str): the path to the file containing the data for the simulation
    - object_name (str): the name of the object we want to calculate the orbit time for

    Returns:
    - float: the time it takes for the object to complete one orbit around a central body
    """
    # read the file and store the data in a list
    ...
    # Decide whether to use multi-threading.
    # If you want to use multi-threading, set multi_processing_enable = True
    # If you don't want to use multi-threading, set multi_processing_enable = False
    multi_processing_enable = True
    # Use multi-threading may speed up the calculation, but also use more memory. If you have enough memory, you can try it.
    # It doesn't mean you always get a better and faster result if you use multi-threading.
    # Sequential execution may be faster than multi-threading in some cases. And achieve better accuracy.
    # Sequential execution still the most stable way to get the result.
    
    if multi_processing_enable:
        # could import concurrent.futures to speed up the calculation, achieve multi-threading.
        max_current_task = 4 # define the max number of tasks running at the same time. Depends on your computer.
        # all libraries below are only used for multi-threading.
        import concurrent.futures
        executor = concurrent.futures.ThreadPoolExecutor()
        # divide the epsilons into groups. Each group has at most max_current_task epsilons.
        epsilons_groups = [epsilons[i:i+max_current_task] for i in range(0, len(epsilons), max_current_task)]
        for epsilons_group in epsilons_groups:
            # for each epsilon, calculate the time it takes for the object to complete one orbit around a central body
            # only return the first one that terminates successfully
            # if none of them terminate successfully, return infinity
            tasks = [executor.submit(orbit_time_epsilon, path_to_file, object_position, object_name, object_index, epsilon) for epsilon in epsilons_group]
            for future in concurrent.futures.as_completed(tasks):
                status, orbit_time_ = future.result()
                if status:
                    # if terminate successfully, cancel all other tasks and return the orbit time.
                    for other_future in tasks:
                        other_future.cancel()
                    return orbit_time_
    else:
        # Execute the function orbit_time_epsilon for each epsilon in the list epsilons sequentially.
        for epsilon in epsilons:
            # get the time it takes for the object to complete one orbit around a central body using the current epsilon
            status, orbit_time_ = orbit_time_epsilon(path_to_file, object_position, object_name, object_index, epsilon)
            # if terminate successfully, return the orbit time. Otherwise, try another epsilon.
            if status:
                return orbit_time_
