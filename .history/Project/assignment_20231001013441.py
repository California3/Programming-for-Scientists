...
def orbit_time(path_to_file, object_name):
    ...
    for epsilons_group in epsilons_groups:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            results = [executor.submit(orbit_time_epsilon, path_to_file, object_position, object_name, object_index, epsilon) for epsilon in epsilons_group]
            for future in concurrent.futures.as_completed(results):
                status, orbit_time_ = future.result()
                if status:
                    for other_future in results:
                        other_future.cancel()
                    return orbit_time_
