...
def orbit_time(path_to_file, object_name):
    ...
    while(True):
        ...
        # object_position_first = position_forward[0, object_index*2:object_index*2+2]

        distance = np.sqrt((object_position_after[0] - object_position[0])**2 + (object_position_after[1] - object_position[1])**2)
        # step_distance = np.sqrt((object_position_first[0] - object_position[0])**2 + (object_position_first[1] - object_position[1])**2)

        if last_distance is not None:
            change_in_distance = distance - last_distance
            if last_change_in_distance is not None:
                if last_change_in_distance <= 0 and change_in_distance >= 0:
        ...
        if not restart:
            ...
            step = step + step_by_step
        print("Current T:", T, "Current step:", step, "Current distance:", distance, "Current step_by_step:", step_by_step)
