...
def orbit_time(path_to_file, object_name):
    ...
    last_not_over = 0.0
    while True:
        position_forward = forward_simulation(path_to_file, T_step * int(step), int(step))
        ...
        if distance <= epsilon:
            print(step, object_name, "small")
            last_not_over = step
            if last_over is None:
                step = step * 2
            elif last_over - last_not_over <= 1:
                return last_not_over * T_step
            else:
                step = ((step + last_over) / 2)
        else:
            last_over = step
            step = ((step + last_not_over) / 2)
            print(step, object_name, "large")
