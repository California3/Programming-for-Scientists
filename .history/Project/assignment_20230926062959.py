...
def forward_simulation(path_to_file, total_time, num_steps):
    position_forward, position_current, velocity_current, acceleration_current = forward_simulation_speedup(path_to_file, 1, total_time / num_steps)
    position_forward2 = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, num_steps - 1, path_to_file, total_time / num_steps)[0]
...
def forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, extend_steps, path_to_file, t_split):
    G_const = 6.67e-11
    data = get_data(path_to_file)
...
def max_accurate_time(path_to_file, epsilon):
    ...
    position_forward_T_1, position_current, velocity_current, acceleration_current = forward_simulation_speedup(path_to_file, 1, 1)
    T_cal_max = 10
    position_forward_T_x, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, T_cal_max - 1, path_to_file, 1)
    ...
    while(True):
        if T <= T_cal_max:
            ...
            position_forward_T_x2, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, T  - T_cal_max, path_to_file, 1)
        ...
        # print(epsilon, T, last_not_over, last_over)
...
def orbit_time(path_to_file, object_name):
    ...
    position_forward_T_1, position_current, velocity_current, acceleration_current = forward_simulation_speedup(path_to_file, 1, T_step)
    step_max = 10
    position_forward_T_x, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, step_max - 1, path_to_file, T_step)
    position_forward_T_cal = np.vstack((position_forward_T_1, position_forward_T_x))[:, object_index*2:object_index*2+2]

    while(True):
        iter_cnt += 1
        restart = False
        if step_by_step < 500:
            print("\n step_by_step < 500. Terminated. T: ", T, "step: ", step, "step_by_step: ", step_by_step, "\n")
            return T
        T = T_step * step


        if step <= step_max:
            object_position_after = position_forward_T_cal[:step, :]
        else:
            position_forward_T_x2, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, step - step_max, path_to_file, T_step)
            position_forward_T_cal = np.vstack((position_forward_T_cal, position_forward_T_x2[:, object_index*2:object_index*2+2]))
            position_forward_T = position_forward_T_cal[:T, :][-1]
            T_cal_max = T
        
        object_position_after = position_forward[-1, :]
        ...
        if not restart:
            ...
            step = step + step_by_step
        print("Iter: ", iter_cnt,"Current T:", T, "Current step:", step, "Current distance:", distance, "Current step_by_step:", step_by_step)
