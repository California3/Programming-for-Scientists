...
def max_accurate_time(path_to_file, epsilon):
    ...
    while(True):
        if T <= T_cal_max:
            ...
            position_forward_T_x, position_current, velocity_current, acceleration_current = forward_simulation_speedup_extend(position_current, velocity_current, acceleration_current, T  - T_cal_max, path_to_file, 1)
            position_forward_T_cal = np.vstack((position_forward_T_1, position_forward_T_x))
            position_forward_T = position_forward_T_cal[:T, :][-1]
            T_cal_max = T
