...
# Read the data only once. Because the data is the same. 
dataset = None
has_read_dataset = False
def get_data(path_to_file):
    global dataset, has_read_dataset
    if not has_read_dataset:
        dataset = np.loadtxt(path_to_file, skiprows=1, usecols=[_ for _ in range(1,5+1)])
