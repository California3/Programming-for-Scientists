...
def forward_simulation(path_to_file, total_time, num_steps):
    ...
    velocity_current = np.zeros((data.shape[0], 2))
    ...
    velocity_current[:,0] = data[:, 3] + t_split * acceleration_current[:,0]
    velocity_current[:,1] = data[:, 4] + t_split * acceleration_current[:,1] # v1 = v0 + a1 * t

    position = []
    for i in range(data.shape[0]):
        position.append(position_current[i,0])
        position.append(position_current[i,1])
    position_forward[0,:] = np.array(position) # store p1
    
    for step in range(1, num_steps):
        position_next = np.zeros((data.shape[0], 2))
        position_next = position_current + t_split * velocity_current # p2 = p1 + v1 * t

        acceleration_next = np.zeros((data.shape[0], 2)) # a2
        for planet_current in range(data.shape[0]):
            for planet_other in range(data.shape[0]):
                if planet_other == planet_current:
                    continue
        ...
        velocity_next = velocity_current + t_split * acceleration_next # v2 = v1 + a2 * t

        position = []
        for i in range(data.shape[0]):
            position.append(position_next[i,0])
            position.append(position_next[i,1])
        position_forward[step,:] = np.array(position) # store p2
